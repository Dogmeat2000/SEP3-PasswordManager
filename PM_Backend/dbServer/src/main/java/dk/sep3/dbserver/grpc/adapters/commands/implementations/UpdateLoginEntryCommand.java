...
@Component
public class UpdateLoginEntryCommand implements GrpcCommand {

    private final LoginEntryRepositoryService loginEntryServiceImpl;

    @Autowired
    public UpdateLoginEntryCommand(LoginEntryRepositoryService loginEntryServiceImpl) {
    ...
    public GenericResponse execute(GenericRequest request) throws DataIntegrityViolationException, PersistenceException {
        ...
        loginEntry = loginEntryServiceImpl.updateLoginEntry(loginEntry);

        // Translate the response returned from the DB into a gRPC compatible type, before sending back to the client:
        return GenericResponseFactory.buildGrpcGenericResponseWithLoginEntryDTO(200, loginEntry);
    }
}
